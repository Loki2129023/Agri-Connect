...
app.post('/predict', async (req, res) => {
    ...
    try {
        ...
        if (!response.data || typeof response.data.predicted_price === 'undefined') {
            return res.status(500).json({ message: 'Invalid response from prediction server' });
        }

        res.status(200).json(response.data);
    } catch (error) {
        console.error('Error predicting crop price:', error.message);

        // Handle network or parsing errors
        res.status(error.response?.status || 500).json({
            message:
                error.response?.data?.error ||
                "Internal Server Error while predicting crop price",
        });
    }
});
